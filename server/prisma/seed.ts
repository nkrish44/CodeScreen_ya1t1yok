...
async function main() {
  ...
  const prefix = envName === "test" ? "hidden.test-" : "";
  const { shifts } = await import(`./seed/${prefix}shifts`);
  const { workers } = await import(`./seed/${prefix}workers`);
  const { workplaces } = await import(`./seed/${prefix}workplaces`);
